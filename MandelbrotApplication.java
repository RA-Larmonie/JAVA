...
public class MandelbrotApplication extends Application implements EventHandler<ActionEvent> {
    ...
    final static int X_PIXEL = 315; // change this value if you have a slow (or fast...) laptop
    ...
    private void startThreads() {
        ...
        for (int i = 0; i < nrofTiles; i++) {
            for (int j = 0; j < nrofTiles; j++) {
                ...
                //this.mbRunnable = new Mandelbrot(tile, pixelManager);
                
                // TODO: starting threads...
                Thread mbThreads = new Thread(m);
                mbThreads.setName("mbThreads: " + Integer.toString(i)+", " + Integer.toString(j));
                mbThreads.start();
                
                //this.mbRunnable = m;
                //this.mbThread = new Thread(this.mbRunnable, "mb thread: X: " + Integer.toString(i) + ", Y: " + Integer.toString(j));
                //this.mbThread.start();
                //m.calculate();
            }
        }
